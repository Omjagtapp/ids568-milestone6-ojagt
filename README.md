...
User Query → embed_query() ──────────────────────────► similarity search
                                                      ...
                                                      Top-k Chunks
                                                             │
                                                 Ollama LLM (mistral:7b-instruct)
                                                             │
                                                    Grounded Response
```
...
- **Generation:** `mistral:7b-instruct` via Ollama local server; graceful fallback if unavailable
...
1. **Ollama not installed in evaluation environment**: LLM generation falls back to context snippets. All retrieval metrics are real. Deploy Ollama + `mistral:7b-instruct` for full generative evaluation.

2. **No out-of-scope detection**: Queries outside the 15-document knowledge base are answered with lowest-ranked retrieved chunks instead of a "no information available" response. Fix: add a minimum similarity threshold.

3. **Model reload per query**: `embed_query()` reloads `all-MiniLM-L6-v2` on every call in the current implementation. In production, cache the model globally to reduce query latency from ~938 ms to ~5 ms.

4. **Small knowledge base**: 15 documents cover NLP/ML topics only. The system is intended as a prototype demonstrating the RAG pattern, not a production retrieval system.

5. **FAISS brute-force index**: `IndexFlatIP` is exact and memory-resident. For corpora > 100K chunks, switch to `IndexIVFFlat` (with `nlist` parameter tuning) or `IndexHNSWFlat` for sub-linear ANN search.

6. **Heuristic tool selection loops**: task_02 triggered 5 retriever calls due to a comparison-type query that didn't match summarizer keywords. Adding a loop-detection cap (max 3 same-tool calls) resolves this.
